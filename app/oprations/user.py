...
def create_new_wallet(request: User, db: Session = Depends(get_db)):
    ...
    new_user = DbUser(
        ...
        user_registration_date_time=datetime.now(
    ...
    db.query(DbUser).filter(DbUser.user_hash_id == new_user.user_hash_id).update({"user_publicKey": wallet_details["publicKey"], "user_privateKey": wallet_details[
        "privateKey"], "user_mnemonic_key": wallet_details["mnemonic"]["phrase"], "user_address": wallet_details["address"]}, synchronize_session='evaluate')
    db.commit()
    details = db.query(DbUser).filter(
        DbUser.user_hash_id == new_user.user_hash_id).all()
    return details
